...
def plot_pattern(ohlc: pd.DataFrame, pat: XABCDFound, pad=3):
    ...
    ax.text(int((pat.X + pat.B) / 2) - pat.X + pad, (s1[pat.X] + s1[pat.B]) / 2, str(round(xa_ab, 3)), color='orange',
            fontsize='x-large')
    ax.text(int((pat.A + pat.C) / 2) - pat.X + pad, (s2[pat.A] + s2[pat.C]) / 2, str(round(ab_bc, 3)), color='orange',
            fontsize='x-large')
    ax.text(int((pat.B + pat.D) / 2) - pat.X + pad, (s1[pat.B] + s1[pat.D]) / 2, str(round(bc_cd, 3)), color='orange',
            fontsize='x-large')
    ax.text(int((pat.X + pat.D) / 2) - pat.X + pad, (s1[pat.X] + s1[pat.D]) / 2, str(round(xa_ad, 3)), color='orange',
            fontsize='x-large')
...
# COMPARES Retracement ratios
# Actual ratio: What we observe
# Pattern Ratio: The Ideal ratio for that pattern
def get_error(actual_ratio: float, pattern_ratio: Union[float, list, None]):
    ...
    if isinstance(pattern_ratio, list):  # Acceptable range
        ...
        # Makes sure the value is in the range
...
# This is where we look for the pattern types from above
def find_xabcd(ohlc: pd.DataFrame, extremes: pd.DataFrame, err_thresh: float = 0.2):
    # Get the retracement values (see retracement_ratios.py
    ...
    # Prepare the output
    # For each output create two signals concurrent with the candle data
    output = {}
    ...
    # Index of first confirmed directional change. Start main loop here
    first_conf = extremes.index[0]
    # Index of first confirmed top or bottom
    extreme_i = 0

    # Controls the trades
    # TODO: Here is where we could implement a Neural Network?
    # TODO: Change the weight here of when to make a trade
    entry_taken = 0
    pattern_used = None
    ...
    for i in range(first_conf, len(ohlc)):
        ...
        # Get price of current high or low
        ...
        xa_ad_retrace = abs(D_price - extremes.iloc[extreme_i - 2]['ext_p']) / extremes.iloc[extreme_i - 2][
            'seg_height']
...
if __name__ == '__main__':
    ...
    for sigma in sigmas:
        ...
        # Error threshold can be 0.2-0.5
