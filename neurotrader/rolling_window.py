...
def rw_top(data: np.array, curr_index: int, order: int) -> bool:
    ...
    top = True
    # Index of local top is delayed by index to avoid looking into the future
    k = curr_index - order
    v = data[k]
    # Check if there is a point that is greater than it on either side
...
def rw_bottom(data: np.array, curr_index: int, order: int) -> bool:
    ...
    # Index of local bottom is delayed
    bottom = True
    k = curr_index - order
    v = data[k]
    # Check if there is a point less than it on either side
    for i in range(1, order + 1):
        if data[k + i] < v or data[k - i] < v:
            bottom = False
            break
